...
* 学习内容
  * 理论环节
    * 【流畅的Python】 第13、14章
  * 实践环节
    * 【LeetCode】medium 难度题目 8道
      * 字符串:  翻转字符串中的单词、Z字形变换、字符串相乘
      * 矩阵: 有效的数独、生命游戏、旋转图像、矩阵置零
      * 链表：排序链表
* 主要收获
  * 【LeetCode】
    * 翻转字符串中的单词?  简单迭代，递归也可
    * Z字形变换?  迭代， 边界处调转方向
    * 字符串相乘? Python直接调库， C++ 先逐位计算乘积再相加， 模仿竖式计算过程
    * 有效的数独?  迭代记忆每个数字出现的块、行、列， 查找哈希过程中如发现冲突情况， 返回False
    * 生命游戏? 迭代判断分析
    * 旋转图像? 先转置， 再左右对称翻转
    * 矩阵置零？ 先遍历标记， 再遍历置零
    * 排序链表? 借助数组， 先存入数组， 再基于val进行排序。 最后重新组织链表的顺序
* 明日计划
  * 【理论环节】
    * 【概统】 第三章: 多维随机变量及其分布
  * 【实践环节】
